...
const SignupPage = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        const result = await authSignIn("/api/auth/signup", payload);
  ...
  useEffect(() => {
    if (refFromUrl) {
      const digitsOnly = refFromUrl.replace(/\D/g, "").slice(0, 6);

      if (digitsOnly.length === 6) {
        formik.setFieldValue("referralId", digitsOnly);
      }
    }
  }, [refFromUrl]);
          ...
          >
            ...
            >
              ...
              {referralUser && (
                <Typography
                  variant="caption"
                  sx={{ mb: 1, display: "block", color: "success.main" }}
                >
                  Referred by:{" "}
