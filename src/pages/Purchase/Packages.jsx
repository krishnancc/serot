...
function BottomFeature({ icon, title, subtitle, color }) {
  return (
    <Stack
      direction="row"
      spacing={{ xs: 1, sm: 1.2 }}
  ...
  return (
    <Paper
      ...
      sx={{
        ...
        border: `1px solid ${
          item.popular || item.bestValue ? item.borderColor : "#E7EAF0"
        }`,
        background: "#FFFFFF",
      ...
      {(item.popular || item.bestValue) && (
        <Box
          sx={{
            ...
            py: { xs: 0.6, sm: 0.8 },
            color: "#fff",
            background: item.popular ? "#1976F3" : "#F33E54",
      ...
      <Box sx={{ p: { xs: 1.75, sm: 2.3 } }}>
        ...
        >
          ...
          <Box sx={{ minWidth: 0 }}>
            ...
            <Chip
              size="small"
              label={`+ ${item?.roi?.percentage}% ROI Daily`}
              sx={{
                mt: 1,
                height: { xs: 23, sm: 25 },
...
function CartContent({
  ...
  const depositBalance = Number(deposit || 0);
  const totalAmount = Number(subtotal || 0);

  // Balance based on selected wallet
  const selectedBalance =
    walletType === "fund" ? walletBalance : depositBalance;

  const insufficientBalance = selectedBalance < totalAmount;

  const balanceShortage = Math.max(totalAmount - selectedBalance, 0);
  ...
  const handlePurchase = async () => {
    ...
    try {
      setPurchasing(true);
      const paymentSource = walletType === "fund" ? "FUND" : "DEPOSIT";
        ...
        >
          ...
          <Stack spacing={{ xs: 1.2, sm: 1.5 }}>
            ...
            >
              ...
              >
                ROI Benefit ({cart[0]?.roi?.durationDays || 0} Days)
              </Typography>
        ...
        >
          ...
          >
            <Stack direction="row" alignItems="center" spacing={1}>
              ...
              >
                {walletType === "fund" ? "Wallet Balance" : "Wallet Balance"}
              </Typography>
            </Stack>
            ...
            >
              {walletType === "fund" ? fund : deposit} USDT
            </Typography>
          </Stack>
          ...
          {insufficientBalance && cart.length > 0 && (
            ...
            >
              ...
              >
                Insufficient {walletType === "fund" ? "Super" : "deposit"}{" "}
                wallet balance. You need {balanceShortage.toFixed(2)} USDT more
...
export default function Packages({ data, wallet, handleSuccess, progress }) {
  ...
  const deposit = parseFloat(wallet?.depositUsd || 0);
  const packageStyles = [
    {
      color: "#1976F3",
      lightColor: "#EFF6FF",
              ...
              >
                ...
                >
                  ...
                  <Box>
                    ...
                    >
                      Choose a package and start earning daily ROI
                    </Typography>
                  </Box>
                </Stack>
