...
const resolveColumnContent = (column, row) => {
    ...
    if (column.renderCell) {
        return column.renderCell({ row, value, field: column.field, id: row.purchaseId });
    }
    return value ?? '—';
};
...
const PurchaseHistroyTable = ({
    ...
    const pageCount = Math.max(Math.ceil((purchaseRowCount || 0) / (purchasePageSize || 10)), 1);
...
const MobilePurchaseList = ({ rows, loading, titleColumn, detailColumns, pageCount, onPageChange }) => {
            ...
            {paginatedRows.map((row) => (
                ...
                >
                    <Box sx={{ justifyContent: 'space-between' }}>
                        ...
                        {/* <Box sx={{ fontSize: 15, fontWeight: 700, color: '#1E293B', wordBreak: 'break-word' }}>
                            {titleColumn ? resolveColumnContent(titleColumn, row) : row.purchaseId}
                        </Box> */}
                    </Box>
                                ...
                                <Stack spacing={1}>
                                    {detailColumns.map((col) => (
                                        <Stack
                                            ...
                                            direction="row"
                                            alignItems="center"
                                            sx={{ justifyContent: 'space-between', textAlign: 'center', alignItems: 'center' }}
                                            spacing={2}
                                        >
                                            <Typography sx={{ fontSize: 12.5, color: TEXT_MUTED, fontWeight: 500 }}>
                                                {col.headerName}
                                            </Typography>
                                            <Box sx={{ fontSize: 13.5, color: '#334155', fontWeight: 600, textAlign: 'right' }}>
