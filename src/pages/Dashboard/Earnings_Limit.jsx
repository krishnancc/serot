...
const Earnings_Limit = ({ data }) => {
    ...
    const progress =
        overallCap > 0 ? (totalEarned / overallCap) * 100 : 0;

    const theme = useTheme();
    const isMobile = useMediaQuery(theme.breakpoints.down("md"));
    const isXs = useMediaQuery(theme.breakpoints.down("sm"));

    // Keep the CircularProgress `size` prop in sync with the container
    // width below — the SVG doesn't scale itself just because its
    // wrapping Box shrinks, so this is computed instead of hardcoded.
