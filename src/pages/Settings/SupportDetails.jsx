            ...
            {items.map((item) => {
                ...
                return (
                    <Box
                        ...
                        onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && onChange(item.key)}
                        sx={{
                            cursor: 'pointer',
...
function CategoryPicker({ value, onChange, error }) {
    return (
        <Box>
            ...
            {error && (
                <Typography sx={{ ...eyebrowSx, color: 'error.main', mt: 0.75 }}>{error}</Typography>
            )}
        </Box>
    );
}
...
const TicketSchema = Yup.object({
    ...
    description: Yup.string().min(10, 'Message required').required('Message required'),
    category: Yup.mixed()
...
const ReplySchema = Yup.object({
    description: Yup.string().min(2, 'Reply message required').required('Reply message required'),
});
...
const SupportDetails = () => {
        ...
        return (
            <>
                ...
                >
                    ...
                    <Typography color="text.secondary" fontWeight={600} fontSize={{ xs: 13.5, md: 14 }}>
                        Loading Support...
                    </Typography>
                </Box>
            </>
    ...
    return (
        <              >
            ...
            <Box sx={{ display: { xs: "block", sm: "none" }, height: MOBILE_HEADER_OFFSET }} />
            ...
            <LedgerSwitch value={tab} onChange={setTab} ticketCount={tickets.length} />
            ...
            {tab === 0 && (
                <Box component="form" onSubmit={ticketForm.handleSubmit}>
                    ...
                    <TextField
                        ...
                        onChange={ticketForm.handleChange}
                        onBlur={ticketForm.handleBlur}
                        error={!!(ticketForm.touched.description && ticketForm.errors.description)}
                        helperText={ticketForm.touched.description && ticketForm.errors.description}
                        sx={{ ...fieldSx, mb: 3, mt: 1 }}
                    />

                    <Box sx={{ mb: 3 }}>
                        <Typography sx={{ ...eyebrowSx, mb: 1 }}>Evidence (optional)</Typography>
                        ...
                        >
                            ...
                            <Box textAlign="center">
                                ...
                                <Typography sx={{ ...eyebrowSx, mt: 0.5 }}>Attach up to 3 files</Typography>
                            </Box>
                        </Box>
                        ...
                        {images.length > 0 && (
                            <Box sx={{ display: 'flex', gap: 1.5, mt: 2, flexWrap: 'wrap' }}>
                                {previews.map((src, index) => (
                                    ...
                                    >
                                        <img src={src} alt="" width="100%" height="100%" style={{ objectFit: 'cover' }} />
                    ...
                    >
                        {submitting ? <CircularProgress size={22} color="inherit" /> : 'File Ticket'}
                    </Button>
                </Box>
            )}
                ...
                <>
                    {tickets.length === 0 ? (
                        ...
                        <Box>
                            ...
                            {tickets.map((t) => {
                                const token =
                                    STATUS_TOKENS[t.status] || STATUS_TOKENS.UNKNOWN;
            ...
            >
                <DialogTitle sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', pr: 1 }}>
                    <Typography sx={{ fontWeight: 800, letterSpacing: '0.02em', fontSize: { xs: 15, sm: 16 } }}>
                        CASE #{String(activeTicketId ?? '').padStart(4, '0')}
                    </Typography>

                    <IconButton onClick={handleCloseReplyModal} size="small" sx={{ color: TEXT_MUTED, '&:hover': { color: 'text.primary' } }}>
                    ...
                    {loadingDetails && !ticketDetails ? (
                        ...
                        <>
                            ...
                            <Box sx={{ flex: 1, overflowY: 'auto', p: { xs: 2, sm: 2.5 } }}>
                                {Array.isArray(ticketDetails?.messages) && ticketDetails.messages.length > 0 ? (
                                    ticketDetails.messages.map((m) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                <Typography sx={{ ...eyebrowSx, mb: 0.5, fontSize: '0.65rem' }}>
                                                    {isUser ? 'You' : 'Support'}
                                                    {'  ·  '}
                                                ...
                                                <Typography sx={{ whiteSpace: 'pre-wrap', mb: 1, fontSize: { xs: 13.5, sm: 14 } }}>
                                                    {m.message}
                                                </Typography>
                                                ...
                                                {Array.isArray(m.images) && m.images.length > 0 && (
                                                    <Box display="flex" gap={1} flexWrap="wrap">
                                                        {m.images.map((img, idx) => (
                                                            <Box key={idx} sx={{ width: { xs: 64, sm: 76 }, height: { xs: 64, sm: 76 }, borderRadius: 2, border: `1px solid ${BORDER}`, overflow: 'hidden' }}>
                                                                <img src={img} alt="" width="100%" height="100%" style={{ objectFit: 'cover' }} />
                                                            </Box>
                                                        ))}
                                                    </Box>
                                                )}
                            ...
                            {/* ================= REPLY ================= */}
                            <Box component="form" onSubmit={replyForm.handleSubmit} sx={{ p: { xs: 1.75, sm: 2 }, bgcolor: 'background.paper' }}>
                                ...
                                <TextField
                                    ...
                                    error={!!(replyForm.touched.description && replyForm.errors.description)}
                                    helperText={replyForm.touched.description && replyForm.errors.description}
                                    sx={{ ...fieldSx, mb: 2 }}
                                />
                                    ...
                                    <Box display="flex" alignItems="center" gap={1} flexWrap="wrap">
                                        ...
                                        >
                                            ...
                                            <input
                                                ...
                                                onChange={(e) => {
                                                    const files = Array.from(e.target.files);
                                                    setReplyImages((prev) => [...prev, ...files].slice(0, 3));
                                                }}
                                            />
                                        </Button>

                                        <Typography sx={eyebrowSx}>{replyImages.length}/3</Typography>
                                    </Box>

                                    {replyPreviews.length > 0 && (
                                        <Box display="flex" gap={1} mt={1} flexWrap="wrap">
                                            {replyPreviews.map((src, index) => (
                                                <Box key={index} sx={{ position: 'relative', width: 44, height: 44, borderRadius: 2, border: `1px solid ${BORDER}`, overflow: 'hidden' }}>
                                                    <img src={src} alt="" width="100%" height="100%" style={{ objectFit: 'cover' }} />
                                ...
                                >
                                    {submitting ? <CircularProgress size={22} color="inherit" /> : 'Send reply'}
                                </Button>
                            </Box>
                        </>
                    )}
